...
// No. of data show at a time
const ROWS_PER_PAGE = 50;

const DetailedEVRecords = ({ data }) => {
  const [currentPage, setCurrentPage] = useState(1);

  if (!data || data.length === 0) {
    return (
      <div className="bg-white p-4 rounded shadow text-center">
        <p className="text-gray-600">No EV records available to display.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-6 rounded shadow overflow-x-auto md:overflow-auto">
      <h2 className="text-xl font-semibold mb-4 text-gray-800">
        Detailed EV Records (Page {currentPage} of {totalPages})
      </h2>
      <table className="min-w-full divide-y divide-gray-200 text-sm w-full md:table-auto"> 
        ...
        <tbody className="bg-white divide-y divide-gray-100 text-gray-800">
          {currentData.map((ev, index) => (
            <tr key={index} className="md:table-row block md:contents"> 
              <React.Fragment>
...
export default DetailedEVRecords;